...
const RecentOrders = ({ recentOrders, loading }) => {
  ...
  if (!recentOrders || recentOrders.length === 0) {
    return (
      <div style={panel}>
        <p style={{ color: '#aaa', margin: 0, fontSize: '13px' }}>No orders logged yet.</p>
      </div>
    )
  }
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', minWidth: '520px' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #f0f0f0' }}>
              {['Email', 'Product Type', 'Qty', 'Date'].map((h) => (
                <th key={h} style={{
          ...
          <tbody>
            {recentOrders.map((o, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #f7f7f7' }}>
                <td style={{ padding: '9px 10px', fontWeight: '600', color: '#111', whiteSpace: 'nowrap', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {o.email || '—'}
                </td>
                ...
                <td style={{ padding: '9px 10px', color: '#111', fontWeight: '700', whiteSpace: 'nowrap' }}>
                  {o.quantity || 1}
                </td>
...
const Dashboard = () => {
  ...
  return (
    <div style={wrap}>
      ...
      {/* ── New Orders (moved to top per request) ── */}
      <SectionTitle>New Orders (Stripe Checkouts)</SectionTitle>
      <RecentOrders recentOrders={data.recentOrders} loading={loading} />
